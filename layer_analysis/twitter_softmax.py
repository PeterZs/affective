...
SAVE_FILE = SAVE_PATH + "softmax-3.txt"
...
results_file.write("Softmax results using C=2^N, with N = " + str(N_values)+"\n\n")
...
for LAYER_NAME in LAYERS:
    ...
    for subset in SUBSETS:
        ...
        # Train an Softmax for each value of N (C)
        i = 0
        for N in N_values:
            C = 2**N
            # Train Softmax using train data
            softmax = linear_model.LogisticRegression(C=C)
            print 'Training Softmax with C='+str(C)+'...'
            sys.stdout.flush()
            t0 = time.time()
            softmax.fit(data_train, labels_train)
            print ("Training Softmax: %.2f s" %(time.time()-t0))
            sys.stdout.flush()
            
            # Test Softmax
            print 'Testing Softmax with C='+str(C)+'...'
            sys.stdout.flush()
            accuracy = 100.*softmax.score(data_test, labels_test)
    ...
    s = "Best classifier: C=" + str(2**N_values[best_i]) + ", Accuracy=" + str(accuracies[best_i]/len(SUBSETS)) + "\n"
    results_file.write(s)
    print s
    sys.stdout.flush()
